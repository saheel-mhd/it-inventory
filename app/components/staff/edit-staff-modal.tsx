...
type EditStaffModalProps = {
  ...
  onClose: () => void;
  onSaved: (next: { name: string; department: string; departmentId: string }) => void;
};
...
export default function EditStaffModal({
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!updateResponse.ok) {
        setError(updatePayload?.error ?? "Failed to update staff.");
        return;
      }
      ...
      for (const row of cleanedRows) {
        ...
        if (!assignResponse.ok) {
          ...
          setError(assignPayload?.error ?? "Failed to assign product.");
          return;
        }
      }

      const updatedDepartment =
        departments.find((item) => item.id === departmentId)?.name ?? updatePayload?.staff?.department ?? "";
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center">
      <button type="button" className="absolute inset-0 bg-black/50" onClick={() => { resetForm(); onClose(); }} />
